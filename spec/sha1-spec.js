...
describe('SHA-1', function() {
  ...
  it('can calculate digests from byte-arrays', function() {
    expect(sha1.fromArray([])).to.deep.eq([218, 57, 163, 238, 94, 107, 75, 13, 50, 85, 191, 239, 149, 96, 24, 144, 175, 216, 7, 9]);
    expect(sha1.fromArray([97])).to.deep.eq([134, 247, 228, 55, 250, 165, 167, 252, 225, 93, 29, 220, 185, 234, 234, 234, 55, 118, 103, 184]);
    expect(sha1.fromArray([97, 98, 99])).to.deep.eq([169, 153, 62, 54, 71, 6, 129, 106, 186, 62, 37, 113, 120, 80, 194, 108, 156, 208, 216, 157]);
  });
  ...
  it('can calculate HMAC digests from byte-arrays', function() {
    expect(hmac.fromArrays([], [])).to.deep.eq([251, 219, 29, 27, 24, 170, 108, 8, 50, 75, 125, 100, 183, 31, 183, 99, 112, 105, 14, 29]);
    ...
    )).to.deep.eq([222, 124, 155, 133, 184, 183, 138, 166, 188, 138, 122, 54, 247, 10, 144, 112, 28, 157, 180, 217]);
  });
